 ...
 * abstract model and view by allowing clients to mutate its abstract fields
 * directly. But clients must remember to reform the pages in order to
 * synchronize the data between the model and view.
 * 
 * A page’s abstract color is rendered the same no matter what state the page is
 * in. A page’s abstract name is rendered thrice centered at every fourth of the
 * page. The name is rendered with a size depending on the zoom level of that
 * page (it tries to maintain a constant aspect ratio). The drawer name is not
 * rendered. The width and height of the page is rendered differently depending
 * on the zoom level and minimumPixelWidth. Using the zoom level, it tries to
 * maintain a constant aspect ratio but the absolute sizes varies with a
 * bigger/smaller zoom level. the minimumPixelWidth limits the width from going
 * below a certain size, no matter what the system tries to set the abstract
 * width to be. Finally the set of blocks are rendered directly onto the page
 * with the same transformation as the ones imposed on the width and height of
 * the page.
 * 
...
public class Page implements WorkspaceWidget, SearchableContainer,
	...
	public Page(String name, int pageWidth, int pageHeight, String pageDrawer,
			boolean inFullview, Color defaultColor) {
		super();
//		this.defaultColor = defaultColor;
//		this.defaultColor = new Color(204,204,255);
//		tipo beige
//		this.defaultColor = new Color(210,202,195);
		this.defaultColor = new Color(138,138,138);	
		...
		//dejamos la pagina mas simple, sin colapsar 
	...
	/**
	 * TODO: THIS METHOD NOT YET DOCUMENTED OR IMPLEMENTED Removes all the
	...
	public void setPageName(String newName) {
		...
		// iterate through blocks and update the ones that are page label
		// enabled
		for (RenderableBlock block : this.getBlocks()) {
	...
	public void addPixelWidth(int deltaPixelWidth) {
		if (fullview) {
			this
					.setPixelWidth((int) (this.getAbstractWidth() * zoom + deltaPixelWidth));
		}
	}
	 ...
	 *          level. May need to convert form pixel to abstract model.
	 */
	...
	public void reformBlockPosition(RenderableBlock block) {
		// move blocks in
		Point p = SwingUtilities.convertPoint(block.getParent(), block
				.getLocation(), this.pageJComponent);
		if (p.x < block.getHighlightStrokeWidth() / 2 + 1) {
			...
			p = SwingUtilities.convertPoint(block.getParent(), block
					.getLocation(), this.pageJComponent);
		...
		if (p.y < block.getHighlightStrokeWidth() / 2 + 1) {
			...
			block.moveConnectedBlocks();
				...
				.getHeight()) {
			block.setLocation(p.x, this.pageJComponent.getHeight()
	...
	public void blockDropped(RenderableBlock block) {
		...
		block.setLocation(SwingUtilities.convertPoint(oldParent, block
				.getLocation(), this.pageJComponent));
		addBlock(block);
	...
	public void addBlock(RenderableBlock block) {
		...
		// if block has page labels enabled, in other words, if it can, then set
		// page label to this
		if (Block.getBlock(block.getBlockID()).isPageLabelSetByPage())
		...
		// if the block is off the edge, shift everything or grow as needed to
		// fully show it
	...
	public ArrayList<RenderableBlock> loadPageFrom(Node pageNode,
		...
		for (int i = 0; i < pageChildren.getLength(); i++) {
			...
			if (pageChild.getNodeName().equals("PageBlocks")) {
				...
				/*
				 * // now that the blocks are loaded, add them to the page. for
				 * (RenderableBlock rb : loadedBlocks) { if(rb != null){ //add
				 * graphically getRBParent().addToBlockLayer(rb);
				 * rb.setHighlightParent(this.getRBParent());
				 * //System.out.println
				 * ("loading rb to canvas: "+rb+" at: "+rb.getBounds()); //add
				 * internallly Workspace.getInstance().notifyListeners(new
				 * WorkspaceEvent(this, rb.getBlockID(),
				 * WorkspaceEvent.BLOCK_ADDED)); if (importingPage) {
				 * Block.getBlock(rb.getBlockID()).setFocus(false);
				 * rb.resetHighlight(); rb.clearBufferedImage(); } } }
				 * 
				 * //now we need to redraw all the blocks now that all
				 * renderable blocks //within this page have been loaded, to
				 * update the socket dimensions of //blocks, etc. for
				 * (RenderableBlock rb : this.getTopLevelBlocks()) {
				 * rb.redrawFromTop(); } this.pageJComponent.revalidate();
				 * this.pageJComponent.repaint();
				 */
	...
	public void addLoadedBlocks(Collection<RenderableBlock> loadedBlocks,
		...
		for (RenderableBlock rb : this.getTopLevelBlocks()) {
			rb.redrawFromTop();
			if (rb.isCollapsed()) {
				// This insures that blocks connected to a collapsed top level
				// block
				// are located properly and have the proper visibility set.
				// This doesn't work until all blocks are loaded and dimensions
				// are set.
				rb.updateCollapse();
			}
		}
...
//		appendAttribute("page-name", this.getPageName(), buf);
		appendAttribute("page-name", HelperEditor.currentRobotName, buf);
		appendAttribute("page-package", HelperEditor.currentRobotPackage, buf);
		appendAttribute("page-type", HelperEditor.currentRobotType, buf);
	...
	public Object getState() {
		...
		for (RenderableBlock rb : this.getBlocks()) {
			...
			state.blocks.put(rb.getBlockID(), rb.getBlock().getState());
		}
		return state;
	}
	...
	public void loadState(Object memento) {
		...
		if (memento instanceof PageState) {
			...
			Map<Long, Object> blockStates = state.blocks;
			...
			for (RenderableBlock existingBlock : getBlocks()) {
				...
				if (renderableBlockStates.containsKey(existingBlockID)) {
					existingBlock.loadState(renderableBlockStates
							.get(existingBlockID));
//vane uncomment this line
					Block.getBlock(existingBlockID).loadState(blockStates.get(existingBlockID));
					existingBlock.updateConnectors();
					//existingBlock.updateSockets();
			...
			// Now, find all the blocks that don't exist in the save state and
			// flag them to be removed.
			...
			// This loop is necessary to avoid a concurrent modification error
			// that occurs
			// if the loop above removes the block while iterating over an
			// unmodifiable
			// iterator.
		...
		//actualizar el codigo fuente si corresponde
		SourceCode.getInstance().update();
	}
...
class BlockStackSorterUtil {
	...
	// TODO ria for now they are ordered in y-coor order
			// this naive ordering will also fail if two blocks have the same
			// coordinates
			new Comparator<RenderableBlock>() {
				public int compare(RenderableBlock rb1, RenderableBlock rb2) {
					if (rb1 == rb2)
						...
						// translate points to a common reference: the parent of
						// rb1
