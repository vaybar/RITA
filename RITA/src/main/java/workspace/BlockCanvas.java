...
public class BlockCanvas implements PageChangeListener, ISupportMemento  {
    ...
    /**
     * @effects Automatically generates Java Code on pages.
     */
    public void generateJavaCode(){
//    	for(Page page : pages){
    		//TODO aqui se hace para varias paginas, yo voy a manejar una sola
    	if (pages.size()>0)
    		WorkspaceController.refreshCodeRegion(pages.get(0));
...
//        saveString.append("<!DOCTYPE CodeBlocks SYSTEM \"save_format.dtd\">");
        //dtd information is not saved because it can´t be in the correct place
        saveString.append("<CodeBlocks>");
        if(pages.size() > 0){  //TODO ria just do BLOCKS, CHECK OUT HOW SAVING WILL BE LIKE WITH REFACTORING
            saveString.append("<Pages drawer-with-page=\"yes\">"); //should we include drawer-with-page flag?
            for(Page page : pages){
