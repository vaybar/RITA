...
public class BlockStub extends Block {
	...
	/**
	 * Temporary mapping for parent type (caller plugs). TODO remove once
	...
	// PERERATARRAGONA
	public static final String EVENTMETHOD_STUB = "eventmethod";
	// END PERERATARRAGONA

	public static final String GETTER_STUB = "getter";
	public static final String SETTER_STUB = "setter";
	public static final String CALLER_STUB = "caller";
		...
		if (stubGenus.startsWith(GETTER_STUB)) {
			...
			if (parent.getNumSockets() > 0) {
				if (parent.isProcedureParamBlock())
					...
					this.setPlug(parent.getSocketAt(0).getKind(),
							PositionType.SINGLE, parent.getSocketAt(0)
									.getLabel(), false, Block.NULL);
			} else {
				if (parent.getPlug() != null)
					this.setPlug(parent.getPlugKind(), parent.getPlug()
							.getPositionType(), parent.getPlugLabel(), parent
			...
			// TODO: remove the following once BlockUtilities.cloneBlock() is
			// finished
			...
			String kind = parentToPlugType.get(parent.getBlockLabel()
					+ parent.getGenusName());// AQUI SE QUEDA CON EL TIPO NUMBER
			// EN EL PLUG..CUANDO SE DEBERIA
			// VOLAR DE LA TABLA?????
			if (kind != null) {
				// vane
				// removeBeforeAndAfter();
				// TODO ria commented code relates to creating mirror plugs for
			...
			// do nothing for now
		} else if (stubGenus.startsWith(EVENTMETHOD_STUB)) {  // PERERATARRAGONA
			// set plug to be the single socket of parent or plug if parent has
			// no sockets
	...
	/*
	 * Updates the shape of Plug in Stuns of the current Block Label
	 */

	public static void shapePlugChanged(String currentParentName,
			String newKind, Long parentID) {
		String oldKey = currentParentName
				+ Block.getBlock(parentID).getGenusName();
		// String newKey = newParentName +
		// Block.getBlock(parentID).getGenusName();

		// only update if parents name really did "change" meaning the new
		// parent name is
		// different from the old parent name
		// Block currentBlock=Block.getBlock(parentID);
		// if(!currentBlock.getPlugKind().equals(newKind)){
		// parentNameToParentBlock.put(newKey, parentID);

		// update the parent name of each stub
		ArrayList<Long> stubs = parentNameToBlockStubs.get(oldKey);
		BlockGenus bg = BlockGenus.getGenusWithName(Block.getBlock(parentID)
				.getGenusName());
		BlockConnector plugIni = null;
		// Block.getBlock(parentID).getPlug()
		BlockStub.parentPlugChanged(parentID, newKind);
		// for(Long stub : stubs){
		//
		// BlockStub blockStub = ((BlockStub)Block.getBlock(stub));
		//
		...
		// //Block.getBlock(parentID).getPlug()
		// // blockStub.setPlug("", PositionType.SINGLE, "", false, parentID);
		//
		// try{
		//
		// blockStub.getPlug().setKind(newKind);
		// plugIni=blockStub.getPlug();
		...
		// //update block label of each
		// // blockStub.setBlockLabel(newParentName);
		// } catch(Exception e){ //Caller case
		// //plugIni exists
		// //BlockConnector plug=new BlockConnector(plugIni);
		// blockStub.setPlug(plugIni.getKind(), PositionType.SINGLE, "", false,
		// Block.NULL);
		// RenderableBlock.getRenderableBlock(blockStub.getBlockID()).updateConnectors();
		// // ..de Renderable.get.updateConnectors()
		// // blockStub.setPlug(plug);
	...
	public static void parentConnectorsChanged(Long parentID) {
		...
		String key = parent.getBlockLabel()
				+ parent.getGenusName();
		...
		for (Long stub : stubs) {
			...
			if (blockStub.stubGenus.startsWith(CALLER_STUB)) {
				if (RenderableBlock.getRenderableBlock(blockStub.getBlockID())
						.isShowing() && blockStub.hasPlug() && blockStub.getPlug().hasBlock())
			...
			if (parent.isVariableDeclBlock() && blockStub.stubGenus.startsWith(GETTER_STUB) ) {
				blockStub.getPlug().setKind(parent.getSocketAt(0).getKind());
				blockStub.notifyRenderable();
				
			}

			if (parent.isVariableDeclBlock() && blockStub.stubGenus.startsWith(SETTER_STUB)) {
				blockStub.getSocketAt(0).setKind(parent.getSocketAt(0).getKind());
				blockStub.notifyRenderable();
	...
	public static int parentConnectorsChanged(Long parentID,
			Block blockChangedCause) {
		int count = 0;
		String key = Block.getBlock(parentID).getBlockLabel()
				+ Block.getBlock(parentID).getGenusName();
		...
		for (Long stub : stubs) {
			...
			if (blockStub.stubGenus.startsWith(CALLER_STUB)
					&& RenderableBlock.getRenderableBlock(
							blockStub.getBlockID()).isShowing()) {
				count++;
				blockStub.updateConnectors(blockChangedCause);
	...
	public static void parentConnectorsLabelChanged(Long parentID,
			String oldLabel, Block blockChangedCause) {
		String key = Block.getBlock(parentID).getBlockLabel()
				+ Block.getBlock(parentID).getGenusName();
		...
		for (Long stub : stubs) {
			BlockStub blockStub = ((BlockStub) Block.getBlock(stub));
			if (blockStub.stubGenus.startsWith(CALLER_STUB)) {
				blockStub.updateLabelConnectors(oldLabel, blockChangedCause);
				// System.out.println("updated connectors of: "+blockStub);
				blockStub.notifyRenderable();
	...
	public static void parentPlugChanged(Long parentID, String kind) {
		...
		for (Long stub : stubs) {
			...
			if (blockStub.stubGenus.startsWith(CALLER_STUB)) {
				...
				// caso: enganche por socket y plug
				if (blockStub.hasPlug()
						&& Block.getBlock(blockStub.getPlugBlockID()) != null
						&& !blockStub.getPlugKind().equals(kind)) {
					BlockConnector socket = null;
					for (BlockConnector bc : Block.getBlock(
							blockStub.getPlugBlockID()).getSockets()) {
						if (bc.getBlockID() == blockStub.getBlockID()) {
							socket = bc;
							disco = true;
							break;
						}
					...
					// desconecta solo si se cambió el tipo
					if (socket != null && socket.getKind() != null
							&& !socket.getKind().equals(kind)) {
						BlockLink.getBlockLink(
								Block.getBlock(blockStub.getPlugBlockID()),
								Block.getBlock(blockStub.getBlockID()), socket,
								blockStub.getPlug()).disconnect();
						disco = true;
						// BlockLink.brokenSound.play();
					}
				...
				if (!blockStub.hasPlug() && blockStub.hasBeforeConnector()) {
					Block b1 = null;
					Block b2 = null;
					if (blockStub != null
							&& blockStub.getBeforeConnector() != null) {
						b1 = Block.getBlock(blockStub.getBeforeConnector()
								.getBlockID());
						b2 = Block.getBlock(blockStub.getBlockID());
					}
					BlockConnector s1 = null;
					BlockConnector s2 = null;

					if (b1 != null) {
						// s1 = b1.getSocketAt(0);
						// s2 = b1.getPlug();
						// BlockLink.getBlockLink(b1, b2, s1, s2).disconnect();
						s1 = b1.getAfterConnector();
						s2 = b2.getBeforeConnector();
						BlockLink.getBlockLink(b1, b2, s1, s2).disconnect();
						disco = true;
						// BlockLink.brokenSound.play();
				...
				if (!blockStub.hasPlug() && blockStub.hasAfterConnector()) {
					Block b1 = null;
					Block b2 = null;
					if (blockStub != null
							&& blockStub.getAfterConnector() != null) {
						b1 = Block.getBlock(blockStub.getAfterConnector()
								.getBlockID());
						b2 = Block.getBlock(blockStub.getBlockID());
					}
					BlockConnector s1 = null;
					...
					if (b1 != null) {
						...
						// s2 = b1.getPlug();
						// BlockLink.getBlockLink(b1, b2, s1, s2).disconnect();
						s1 = b1.getBeforeConnector();
						s2 = b2.getAfterConnector();
						BlockLink.getBlockLink(b1, b2, s1, s2).disconnect();
						disco = true;
						// BlockLink.brokenSound.play();
	...
	public void updateConnectors() {
		...
		if (parent != null) {
			...
			// if (this.hasPlug()
			// && this.getPlug().getPositionType()
			// .equals(BlockConnector.PositionType.SINGLE)) {
	...
	public void updateLabelConnectors(String oldLabel, Block blockChangedCause) {
		Block parent = getParent();
		if (parent != null) {
			// retrieve sockets from parent and set sockets accordingly
			Iterator<BlockConnector> parentSockets = parent.getSockets()
			...
			// clear all sockets TODO temporary solution
			for (BlockConnector socket : getSockets())
				if (socket.getLabel().equals(oldLabel)
						&& !socket.getLabel().equals(
								blockChangedCause.getBlockLabel())) {
					socket.setLabel(blockChangedCause.getBlockLabel());
					notifyRenderable();
					break;
				}
		}
	...
	public void updateConnectors(Block blockChangedCause) {
		Block parent = getParent();
		if (parent != null) {
			// retrieve sockets from parent and set sockets accordingly
			Iterator<BlockConnector> parentSockets = parent.getSockets()
			...
			for (i = 0; parentSockets.hasNext(); i++) {
				...
				if (parentSocket.getBlockID() != Block.NULL) {
					...
					if (i > this.getNumSockets() - 1) {
						...
						// blocks of parent
						if (parentSocket.getBlockID() != Block.NULL
								&& Block.getBlock(parentSocket.getBlockID()) != null)
							addSocket(parentSocket.getKind(),
									BlockConnector.PositionType.SINGLE,
									Block.getBlock(parentSocket.getBlockID())
											.getBlockLabel(), false, false,
			...
			// BlockConnector.PositionType.SINGLE)
			// && (!blockChangedCause.getGenusName().contains(
			// "proc-param"))) {
			// this.restoreInitConnectors();
			// }
			// }
			...
			if (stubGenus.startsWith(CALLER_STUB)) {
				BlockUtilities.verificarTipoReturn(blockChangedCause, parent, false);
			}
		}
	}
	...
	private void updatePlug(String kind) {
		...
		// Always synchronize! We can't have both a plug and a before.
		removeBeforeAndAfter();
		// TODO ria commented code relates to creating mirror plugs for caller
	 ...
	 */
	public void disconnectBlock(Long id) {
	...
	public Color getColor() {
		...
		return ((Block)parentBlock).getColor();
	}
	...
	public boolean isCommandBlock() {
		// Vane
		BlockGenus bg = BlockGenus.getGenusWithName(Block.getBlock(
				this.getBlockID()).getGenusName());
		return bg.isCommandBlock();
	...
	public static void removeFromPlugMap(String key) {
		parentToPlugType.remove(key);
	}
}
