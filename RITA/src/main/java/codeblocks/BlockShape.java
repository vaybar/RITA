...
public class BlockShape {
    ...
    protected void makeTopSide() {
  		...
  		setEndPoint(gpTop, topLeftCorner, botLeftCorner, true);
    		...
    		//This block "IF..." creates a top curve on block shape!
    		//command socket if necessary 	
//    		if ((block.isCommandBlock() && block.hasBeforeConnector()) || (block.isVariableDeclBlock() && block.hasBeforeConnector()) || (block.isReturnBlock() && block.hasBeforeConnector())
//    				|| (block.isControlBlock() && block.hasBeforeConnector())) {
    		if ((block.isCommandBlock() ) || (block.isVariableDeclBlock() && block.hasBeforeConnector()) || (block.isReturnBlock() && block.hasBeforeConnector())
    				|| (block.isControlBlock() && block.hasBeforeConnector())) {
...
//if (block.getAfterConnector()!=null)			
