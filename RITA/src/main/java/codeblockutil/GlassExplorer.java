...
public class GlassExplorer extends JPanel implements Explorer, FocusListener{
	...
	public void setDrawersCard(List<? extends Canvas> items){
		...
		JLabel label=null;
		// font de los titulos de cada grupo de bloques
		final Font blockGroupTitleFont = UIManager.getDefaults().getFont("Label.font").deriveFont(Font.BOLD, 14);
		if (items.size()==10){ //PERERATARRAGONA
		for(int i = 0; i<items.size(); i++){
			if (i==0){
				label=new JLabel(space + Language.get("robocodeBlock"));
			}
			if (i==5){
				label=new JLabel(space + Language.get("basicBlock"));
			}
			if (i==8){
				label=new JLabel(space + Language.get("newCodeBlock"));
			}
			if (i==0 || i==5 || i==8){
				label.setAlignmentX(CENTER_ALIGNMENT);
				// agregar una separacion entre grupos de bloques mediante un borde transparente en el titulo del bloque
				label.setBorder(BorderFactory.createEmptyBorder(8,2,0,0));
				label.setFont(blockGroupTitleFont);
	...
	public void reformView(){
		...
		if(this.getHeight()<this.drawers.size()*35){
			retardedPane.setPreferredSize(new Dimension(0, 0));
		}else{
			retardedPane.setPreferredSize(new Dimension(0, this.getHeight()-this.drawers.size()*35));
		}
	}
    /**
