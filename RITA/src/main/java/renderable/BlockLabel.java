...
public class BlockLabel implements MouseListener, MouseMotionListener,
	...
	public final static Font blockFontSmall_Bold = new Font("Arial", Font.BOLD,
			7);
	public final static Font blockFontMedium_Bold = new Font("Arial",
			Font.BOLD, 10);
	public final static Font blockFontLarge_Bold = new Font("Arial", Font.BOLD,
			12);
	public final static Font blockFontSmall_Plain = new Font("Arial",
			Font.PLAIN, 7);
	public final static Font blockFontMedium_Plain = new Font("Arial",
			Font.PLAIN, 10);
	public final static Font blockFontLarge_Plain = new Font("Arial",
			Font.PLAIN, 12);
	...
	public BlockLabel(String initLabelText, BlockLabel.Type labelType,
		...
		this(initLabelText, labelType, isEditable, -1, false, tooltipBackground,false); //,false
	}

	public BlockLabel(String initLabelText, BlockLabel.Type labelType,
			boolean isEditable, long blockID, boolean hasComboPopup,
			Color tooltipBackground,boolean isLoading) { //,boolean isLoading
		...
		widget.setNumeric(Block.getBlock(this.blockID).getGenusName().equals(
				"number"));
		...
		// set initial text
		//se pregunta si es el momento de la carga porque sino, pierde los sockets!
//		if (!isLoading)
			widget.updateLabelText(initLabelText,isLoading);
	...
	protected void textChanged(String text) {
		String oldLabel=Block.getBlock(blockID).getBlockLabel();
		if ((this.labelType.equals(BlockLabel.Type.NAME_LABEL) || this.labelType
			...
			if (this.labelType.equals(BlockLabel.Type.NAME_LABEL)
				...
				if (!Block.getBlock(blockID).isDataBlock() && text.contains(" ")){
					 new MessageDialog(Language.get("textChanged.blank"),MessageType.ERROR);
					 return;
				}
				boolean paramOK=validateParams(text);
				if (!paramOK){
					 new MessageDialog(Language.get("textChanged.param"),MessageType.ERROR);
					 return;
				}
			...
			if (plug != null && plug.getBlockID() != Block.NULL) {
				if (Block.getBlock(plug.getBlockID()) != null) {
					if (Block.getBlock(plug.getBlockID())
						...
						// again.
						if (Block.getBlock(blockID).getGenusName().contains(
								"proc-param")) {
							BlockStub.parentConnectorsLabelChanged(
									plug.getBlockID(), widget.getLabelBeforeEdit(), Block.getBlock(blockID));
						} else if (RenderableBlock.getRenderableBlock(
	...
	private boolean validateParams(String text) {
		Block block = Block.getBlock(this.blockID);
		if (block.getGenusName().contains("proc-param")) {
			// this is the custom method
			Block method = Block.getBlock(block.getPlug().getBlockID());
			if (method != null) {
				for (BlockConnector bc : method.getSockets()) {
					if (bc.getBlockID()!=Block.NULL && !bc.getBlockID().equals(block.getBlockID())) {
						if (Block.getBlock(bc.getBlockID())!=null && Block.getBlock(bc.getBlockID()).getBlockLabel()
								.equals(text))
							return false;
					}
				}
	...
	protected boolean textValid(String text) {
		return !text.equals("") && BlockUtilities.isLabelValid(blockID, text);
	}

	public void mouseClicked(MouseEvent e) {
		if (!((e.getClickCount() == 1) && widget.isEditable())) {
			RenderableBlock.getRenderableBlock(blockID).processMouseEvent(
					SwingUtilities.convertMouseEvent(widget, e, widget
							.getParent()));
		}
	}
