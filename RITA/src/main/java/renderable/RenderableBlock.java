...
public class RenderableBlock extends JComponent implements SearchableElement,
	...
	// keep the number related to the previous RenderableBlock with the same
	// label
	private int labelInstanceNumber = 0;

	public int getLabelInstanceNumber() {
		return labelInstanceNumber;
	}

	public void setLabelInstanceNumber(int labelInstanceNumber) {
		this.labelInstanceNumber = labelInstanceNumber;
	}
	 ...
	 */
	protected RenderableBlock(WorkspaceWidget parent, Long blockID,
			boolean isLoading) {
		super();
		/*
		...
		for (BlockImageIcon img : getBlock().getInitBlockImageMap().values()) {
			imageMap.put(img.getImageLocation(), new BlockImageIcon(img
					.getImageIcon(), img.getImageLocation(), img.isEditable(),
					img.wrapText()));
...
//		if (!isLoading ||(isLoading && BlockStub.getBlock(this.getBlockID())==null))
			synchronizeSockets();
	...
	private Dimension calcStackDimensions(RenderableBlock rb) {
		if (rb.getBlock().getAfterBlockID() != Block.NULL) {
			...
			return new Dimension(Math.max(rb.getBlockWidth()
					+ rb.getMaxWidthOfSockets(rb.getBlockID()), dim.width), rb
					.getBlockHeight()
					+ dim.height);
		} else
		...
		for (int i = 0; i < getBlock().getNumSockets(); i++) {
			...
			if (tag == null) {
				...
				if (SocketLabel.ignoreSocket(socket)) {
					...
					SocketLabel label = new SocketLabel(socket, socket
							.getLabel(), BlockLabel.Type.PORT_LABEL, socket
				...
				if (!SocketLabel.ignoreSocket(socket)) {
					...
					if (label == null) {
						label = new SocketLabel(socket, socket.getLabel(),
								BlockLabel.Type.PORT_LABEL, socket
										.isLabelEditable(), blockID);
	...
	public int accomodateLabelsWidth() {
		...
		for (ConnectorTag tag : socketTags) {
			SocketLabel label = tag.getLabel();
			if (label != null)
				maxSocketWidth = Math.max(maxSocketWidth, label
						.getAbstractWidth());
		}
		if (getBlock().hasPageLabel()) {
			width += Math.max(blockLabel.getAbstractWidth(), pageLabel
					.getAbstractWidth())
					+ maxSocketWidth;
	...
	public void updateSockets() {
		...
		for (BlockConnector bc: b.getSockets())
			socketTags.add(new ConnectorTag(bc));
...
*/
	...
	//<!--PERERATARRAGONA ---> Cambiamos los valores de 40 a 0 por error con los colores
	private final int colorRedOffset=40;
	private final int colorGreenOffset=40;
	private final int colorBlueOffset=40;
	/**
	 * Este método devuelve el valor de color del bloque. En caso que el bloque sea un getter o setter
	 * el bloque tendrá un color de tono más claro que el bloque a partir del cual se origina.
	 * */
	public Color getBLockColor() {
		if (getBlock().getGenusName().contains(BlockStub.GETTER_STUB) || getBlock().getGenusName().contains(BlockStub.SETTER_STUB)){
			Color originalColor=getBlock().getColor();
			Color newColor = new Color(originalColor.getRed()+colorRedOffset,originalColor.getGreen()+colorGreenOffset,originalColor.getBlue()+colorBlueOffset);
			return newColor;
		}
		else return getBlock().getColor();
	}
		...
		if (!linkedDefArgsBefore && getBlock().hasDefaultArgs()) {
			...
			while (ids.hasNext() && sockets.hasNext()) {
				...
				if (id != Block.NULL) {
					...
					RenderableBlock arg = new RenderableBlock(this
							.getParentWidget(), id);
	...
	private void updateSocketSpace(BlockConnector connectedSocket,
		...
		if (!isConnected) {
			// remove the mapping
			//vane
			//if (this.getConnectorTag(connectedSocket)!=null)
			...
			// Block.getBlock(connectedToBlockID)
	...
	public void blockDisconnected(BlockConnector disconnectedSocket, Block blockCause) {
		// notify block first so that we will only need to repaint this block
		// once
		getBlock().blockDisconnected(disconnectedSocket,blockCause);
	...
	private void updateBuffImg() {
		...
		// DRAW BLOCK IMAGES
		repositionBlockImages(blockArea.getBounds().width, blockArea
				.getBounds().height);
	}
	...
	private void repositionBlockImages(int width, int height) {
		...
		for (BlockImageIcon img : imageMap.values()) {
			...
			if (img.getImageLocation() == BlockImageIcon.ImageLocation.CENTER) {
				...
				imgLoc
						.setLocation(width - margin - icon.getIconWidth(),
								margin);
	...
	public void addComment() {
		if (hasComment()) {
			...
			int y = this.getY() - 40;
			comment = new Comment(Language.get("emptyComment"), this, this.getBlock().getColor(), zoom);
		...
		if (comment != null) {
			...
			if (commentLabel != null) {
				location.translate(commentLabel.getX() - 2,
						commentLabel.getY() - 2);
				location.translate(commentLabel.getWidth() / 2, commentLabel
						.getHeight() / 2);
			}
		}
	...
	public void moveConnectedBlocks() {
		...
		for (BlockConnector socket : BlockLinkChecker.getSocketEquivalents(b)) {
			...
			if (socket.hasBlock()) {
				...
				rb.setLocation((int) Math.round((float) myScreenOffset.getX()
						+ socketLocation.getX()
						- (float) otherScreenOffset.getX()
						- plugLocation.getX()), (int) Math
						.round((float) myScreenOffset.getY()
								+ socketLocation.getY()
...
//		SourceCode.getInstance().refreshOK=true;
...
//		RenderableBlock rBlock=(RenderableBlock)e.getSource();
//		if (!existsInstances(rBlock,"method"))
			super.processMouseEvent(e);
//		else {new MessageDialog(Language.get("error.maxInstances"), MessageType.INFO);
//		return;}
	}
	private boolean existsInstances(RenderableBlock rBlock, String kind) {
    	Block block=Block.getBlock(rBlock.getBlockID());
    	if (BlockGenus.getGenusWithName(block.getGenusName()).isMethodDeclBlock() &&  (Workspace.getInstance().getBlocksFromGenus(block.getGenusName())).iterator().hasNext())
    		return true;
    	else return false;
    }
	...
	public void mouseDragged(MouseEvent e) {
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			if (Workspace.getInstance().getMiniMap().contains(pp)) {
				Workspace.getInstance().getMiniMap().blockDragged(this,
						e.getPoint());
			...
			if (!dragging) {
				...
				if (plug != null && plug.hasBlock()) {
					...
					RenderableBlock.getRenderableBlock(parent.getBlockID())
							.blockDisconnected(socket,block);
	...
	private BlockConnector findSocket(Block receiverValue, Block returnBlock) {
		for (BlockConnector bc : receiverValue.getSockets()) {
			if (bc.getBlockID().equals(returnBlock.getBlockID())) {
				return bc;
			}
		}
		return null;
	...
	public void mouseClicked(MouseEvent e) {
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			Workspace.getInstance().notifyListeners(
					new WorkspaceEvent(this.getParentWidget(), this
							.getBlockID(),
							WorkspaceEvent.BLOCK_SELECTED_HIGHLIGHT_CODE));
		}
	}
