...
import rita.ui.sourcecodepane.ReadOnlySourceCodePane.RegExpHashMap;
...
public class DefaultSyntaxColorizer extends SyntaxColorizer {
    ...
    protected StyledTextPane component;

    public DefaultSyntaxColorizer(StyledTextPane aComponent) {
...
//TODO setKeywordBackgroundColor, setKeywordItalic, setKeywordBold, setKeywordUnderline
    public void setKeywordColor(HashMap<String, Color> keywords) {
        ...
        for (Entry<String, Color>  kv: keywords.entrySet()) {
            if (!colors.containsKey(kv.getValue())) {
                MutableAttributeSet kw = new SimpleAttributeSet();
                StyleConstants.setForeground(kw, kv.getValue());
                colors.put(keywords.get(kv.getKey()), kw);
            }
        }
